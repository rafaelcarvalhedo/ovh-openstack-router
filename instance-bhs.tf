resource "openstack_compute_keypair_v2" "test-keypair-bhs" {
  name       = "carvalhedo-bhs"
  public_key = "ssh-rsa  rafaelsousa@MacBook-Air-de-Rafael.local"
}

resource "openstack_compute_instance_v2" "basic-bhs" {
  name            = "basic-bhs"
  image_name  = "Debian 11"
  flavor_name       = "s1-2"
  key_pair        = openstack_compute_keypair_v2.test-keypair-bhs.name
  security_groups = ["default"]
  
  metadata = {
    this = "that"
  }

  network {
    name = openstack_networking_network_v2.netowrk_app.name
  }
 
}

resource "openstack_compute_instance_v2" "basic-bhs2" {
  name            = "basic-bhs2"
  image_name  = "Debian 11"
  flavor_name       = "s1-2"
  key_pair        = openstack_compute_keypair_v2.test-keypair-bhs.name
  security_groups = ["default"]
  
  metadata = {
    this = "that"
  }

  network {
    name = openstack_networking_network_v2.network_db.name
  }
 
}